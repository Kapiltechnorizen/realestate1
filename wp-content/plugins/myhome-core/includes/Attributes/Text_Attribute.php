...
class Text_Attribute extends Attribute {
	 ...
	 */
	public function get_values( $all_values = false, $order_by = Term_Factory::ORDER_BY_COUNT, $order = Term_Factory::ORDER_DESC ) {
		$values         = array();
		...
		if ( $default_values == Attribute_Options_Page::MOST_POPULAR && ! $all_values ) {
			$terms = Term_Factory::get( $this, $this->get_most_popular_limit(), $order_by, $order );
		} else {
			$terms = Term_Factory::get( $this, Term_Factory::ALL, $order_by, $order );
		}
	 ...
	 * @return array
	 */
	public function get_vc_control(
		$fields
	) {
		$terms   = Term_Factory::get( $this );
