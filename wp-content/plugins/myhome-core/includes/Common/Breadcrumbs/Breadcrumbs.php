...
class Breadcrumbs {
	...
	public function __construct() {
		...
		for ( $i = 0; $i < $counter; $i ++ ) {
			...
			$value = get_query_var( $attributes_list[ $attribute_id ]->get_slug(), '' );
			if ( empty( $value ) ) {
				break;
			}
			...
			$breadcrumb          = new Breadcrumb( $this->current_term->get_name(), $archive_link, $attributes_list[ $attribute_id ], $this->current_term, $this->ids );
	...
	public function get_child_attribute() {
		...
		if ( ! isset( \MyHomeCore\My_Home_Core()->settings->props[ $child_key ] ) || empty( \MyHomeCore\My_Home_Core()->settings->props[ $child_key ] ) ) {
			return false;
		}
