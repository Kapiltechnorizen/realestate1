...
class Field {
	...
	public function get_data() {
		if ( $this->attribute instanceof Text_Attribute ) {
			...
			if ( $order_by == 'name' ) {
				$values = $this->attribute->get_values( false, Term_Factory::ORDER_BY_NAME, Term_Factory::ORDER_ASC );
			} else {
		...
		if ( $this->attribute instanceof Keyword_Attribute && $this->attribute->get_suggestions() ) {
			...
			if ( ! empty( $attribute_id ) ) {
				...
				if ( $attribute != false && $attribute instanceof Text_Attribute ) {
					...
					$terms         = Term_Factory::get( $attribute );
			...
			$data['api_endpoint'] = site_url() . '/wp-json/myhome/v1/suggestions';
		}
