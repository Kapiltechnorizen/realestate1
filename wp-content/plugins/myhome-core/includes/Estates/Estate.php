...
class Estate {
	...
	public function get_offer_type() {
		return Attribute_Factory::get_offer_type()->get_estate_values( $this );
	}
		...
		foreach ( $currencies as $currency ) {
			...
			foreach ( $offer_types as $offer_type ) {
				...
				foreach ( $price_keys as $price_key ) {
					...
					$price                = floatval( get_post_meta( $this->post->ID, 'estate_attr_' . $field_key, true ) );
	...
	public function has_image() {
		return has_post_thumbnail( $this->post->ID ) || $this->has_gallery();
	}
	...
	public function get_image_id() {
		...
		if ( empty( $image_id ) && $this->has_gallery() ) {
	...
	public function get_modified_date() {
		...
		return date( $date_format, strtotime( $this->post->post_modified ) );
	}
