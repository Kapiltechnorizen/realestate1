...
class Property_Manager {
	...
	public function save_draft() {
		...
		if ( is_user_logged_in() ) {
			...
			if ( empty( $user_id ) ) {
				wp_die( esc_html__( 'Temporary draft user not set. You can set it in the MyHome Theme options (Users -> Frontend settings)', 'myhome-core' ) );
			}
	...
	public function delete_property() {
		...
		if ( ! $property instanceof \WP_Post || ( ! in_array( $property->post_author, $ids ) && $property->post_author != $user->ID && empty( $all ) && ! in_array( 'super_agent', $user->roles ) && ! in_array( 'administrator', $user->roles ) ) ) {
			wp_die();
		}
	...
	public function edit_property_form() {
		...
		try {
			...
			if ( $property->get_user()->get_ID() != get_current_user_id() && empty( $all ) && ! in_array( 'super_agent', $user->roles ) && ! in_array( 'administrator', $user->roles ) && ! in_array( $property->get_user()->get_ID(), $ids ) ) {
				wp_die();
			}
