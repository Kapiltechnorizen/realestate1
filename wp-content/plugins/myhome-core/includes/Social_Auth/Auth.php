	...
	public function login() {
		...
		if ( $network == 'yahoo' ) {
			$config = array(
				...
				'keys'     => array(
					'id'     => \MyHomeCore\My_Home_Core()->settings->get( 'social_login-' . $network . '-id' ),
					'secret' => \MyHomeCore\My_Home_Core()->settings->get( 'social_login-' . $network . '-secret' )
				)
			);
		} else {
			$config = array(
				'callback' => admin_url( 'admin-post.php?action=myhome_social_login_callback_' . $network ),
				'keys'     => array(
					'key'    => \MyHomeCore\My_Home_Core()->settings->get( 'social_login-' . $network . '-id' ),
					'secret' => \MyHomeCore\My_Home_Core()->settings->get( 'social_login-' . $network . '-secret' )
				)
			);
		}
