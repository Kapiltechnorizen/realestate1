...
class Agents_Manager {
	...
	public function saved_searches_menu_page() {
		...
		<div class="mh-saved-search-page">
			...
			<br>
			<br><?php esc_html_e( 'Last time emails where changed', 'myhome-core' ); ?>: <?php echo esc_html( $last_check ); ?>
			<a class="button button-primary" href="<?php echo esc_url( admin_url( 'admin-post.php?action=myhome_saved_searches_check' ) ); ?>">
			...
			<br>
			<a href="<?php echo esc_url( admin_url( 'admin-post.php?action=myhome_saved_searches_hash' ) ); ?>" class="button">
	...
	public function remove_agent() {
		...
		if ( ! $agent instanceof \WP_User || ! in_array( 'agent', $agent->roles ) ) {
			wp_die();
		}
	...
	public function invite_agent() {
		...
		echo json_encode( array(
			'success' => true,
			'message' => esc_html__( 'Agent successfully invited, awaiting reply', 'myhome-core' )
		) );
		wp_die();
	}
	...
	public static function send_activation_link( User $user ) {
		...
		return wp_mail( $user->get_email(), $subject, $message, $headers );
	}
	...
	public static function get_account_types() {
		\MyHomeCore\My_Home_Core()->settings->get( '' );
		...
		foreach ( $types as $type_key => $type ) {
			if ( ! isset( \MyHomeCore\My_Home_Core()->settings->props[ 'mh-agent_role-' . $type_key ] ) || ! empty( \MyHomeCore\My_Home_Core()->settings->props[ 'mh-agent_role-' . $type_key ] ) ) {
				$enabled_types[ $type_key ] = \MyHomeCore\My_Home_Core()->settings->get( 'agent_role-' . $type_key . '-display' );
			}
		}
